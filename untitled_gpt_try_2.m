%function demoDopplerGroundReceiverNewSatData_GeodeticOutput()
    ...
    lat_rec_deg_guess = 40.75;
    lon_rec_deg_guess = 120.65;
    alt_rec_km_guess  = 0;
    ...
    x0 = [geodetic2ecef(lat_rec_guess, lon_rec_guess, alt_rec_guess, a, e2); deltaR_true; [0; 0; 0]; dDeltaRdt_true];
    ...
    for j = 1:nSat
        ...
        term = hat_rho' * relative_v + c*x_true(8) - c*dot_delta_sat(j);
    ...
    % if doIteration
        ...
        maxIter = 50; tol = 1e-8;
        x = x0;
        [ x_est, iter ] = newtonIteration(x, r_sat, v_sat, D, c, lambda, maxIter, tol);
...
%% ------- 牛顿–高斯迭代函数 -------
function [x, iters, err_hist] = newtonIteration(x, r_sat, v_sat, D, c, lambda, maxIter, tol)
% newtonIteration   使用牛顿–高斯方法迭代求解多普勒方程组
% 输入:
%   x0       初始状态向量 [r(3); deltaR; v(3); dDeltaRdt]
%   r_sat    3×8 卫星 ECEF 位置
%   v_sat    3×8 卫星 ECEF 速度
%   D        8×1 多普勒观测 (Hz)
%   c        光速 (m/s)
%   lambda   载波波长 (m)
%   maxIter  最大迭代次数
...
%   err_hist 每次迭代的增量范数
    ...
    for k = 1:maxIter
        % 计算残差和雅可比
        f_vec = dopplerResidual8(x, r_sat, v_sat, D, c, lambda);
        J     = computeJacobian8(x, r_sat, v_sat, c);
        % 增量 dx
        dx = - J \ f_vec;
        x  = x + dx;
        err_hist(k) = norm(dx);
...
%% ------- 雅可比函数 (8×8) -------
function J = computeJacobian8(x, r_sat, v_sat, c)
% computeJacobian8  计算多普勒方程组的雅可比矩阵
% 输入:
%   x      8×1 状态向量 [r(3); deltaR; v(3); dDeltaRdt]
%   r_sat  3×8 卫星 ECEF 位置
%   v_sat  3×8 卫星 ECEF 速度
%   c      光速 (m/s)
% 输出:
%   J      8×8 雅可比矩阵, 每行对应一颗卫星方程对各未知量的偏导数
    % 提取状态量
    r        = x(1:3);       % 接收机位置
    %deltaR   = x(4);        % 接收机钟偏 (在此模型中残差对其偏导为0)
    v        = x(5:7);       % 接收机速度
    %dDeltaRdt = x(8);       % 钟偏率 (仅影响残差函数常数项)
    ...
    for j = 1:8
        ...
        relv   = v - v_sat(:,j);
        % d(hatrho)/dr
        dH = eye(3)/norm_d - (dvec*dvec')/(norm_d^3);
        % ∂f/∂r = dH * (v - v_j)
        J(j,1:3) = (dH * relv)';
        % ∂f/∂deltaR = 0
        J(j,4)   = 0;
...
%% ------- 残差函数 dopplerResidual8 -------
function f_vec = dopplerResidual8(x, r_sat, v_sat, D, c, lambda)
% dopplerResidual8   计算8个多普勒方程的残差向量
% 输入:
%   x      8×1 状态向量 [r(3); deltaR; v(3); dDeltaRdt]
%   r_sat  3×N 卫星ECEF位置
%   v_sat  3×N 卫星ECEF速度
...
%   f_vec  N×1 残差向量
    ...
    for j = 1:Nsat
        dvec    = r - r_sat(:,j);
        hatrho  = dvec / norm(dvec);
        relv    = v - v_sat(:,j);
        % f_j = hatrho'*(v - v_j) + c*dDelta + lambda*D_j
        f_vec(j) = hatrho' * relv + c*dDelta + lambda * D(j);
    end
end
